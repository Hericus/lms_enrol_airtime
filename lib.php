 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_airtime_plugin extends enrol_plugin {
    ...
    public function can_delete_instance($instance) {
        ...
        return has_capability('enrol/airtime:config', $context);
    }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $coursecontext) or !has_capability('enrol/airtime:config', $coursecontext)) {
            return false;
        }
    ...
    public function add_instance($course, array $fields = null) {
        ...
        if (!empty($fields['customint2']) && $fields['customint2'] == AIRTIME_CREATE_GROUP) {
            // Create a new group for the cohort if requested.
            $context = context_course::instance($course->id);
            require_capability('moodle/course:managegroups', $context);
            $groupid = enrol_airtime_create_new_group($course->id, $fields['customint1']);
        ...
        enrol_airtime_sync($trace, $course->id);
        $trace->finished();
    ...
    public function update_instance($instance, $data) {
        ...
        if ($data->roleid != $instance->roleid) {
            ...
            $params = array(
                ...
                'component' => 'enrol_airtime',
                'itemid' => $instance->id
            );
            role_unassign_all($params);
        }
        // Create a new group for the cohort if requested.
        if ($data->customint2 == AIRTIME_CREATE_GROUP) {
            require_capability('moodle/course:managegroups', $context);
            $groupid = enrol_airtime_create_new_group($instance->courseid, $data->customint1);
        ...
        require_once("$CFG->dirroot/enrol/airtime/locallib.php");
        $trace = new null_progress_trace();
        enrol_airtime_sync($trace, $instance->courseid);
        $trace->finished();
    ...
    protected function get_group_options($coursecontext) {
        ...
        if (has_capability('moodle/course:managegroups', $coursecontext)) {
            $groups[AIRTIME_CREATE_GROUP] = get_string('creategroup', 'enrol_airtime');
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $coursecontext) {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_airtime'), $options);
        ...
        $mform->addElement('select', 'roleid', get_string('assignrole', 'enrol_airtime'), $roles);
        $mform->setDefault('roleid', $this->get_config('roleid'));
        $groups = $this->get_group_options($coursecontext);
        $mform->addElement('select', 'customint2', get_string('addgroup', 'enrol_airtime'), $groups);
    }
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        $sql = "roleid = :roleid AND customint1 = :customint1 AND courseid = :courseid AND enrol = 'airtime' AND id <> :id";
        if ($DB->record_exists_select('enrol', $sql, $params)) {
            $errors['roleid'] = get_string('instanceexists', 'enrol_airtime');
        }
 ...
 * @return bool
 */
function enrol_airtime_allow_group_member_remove($itemid, $groupid, $userid) {
    return false;
}
 ...
 */
function enrol_airtime_create_new_group($courseid, $cohortid) {
    global $DB, $CFG;
    ...
    $a->name = $groupname;
    $a->increment = '';
    $groupname = trim(get_string('defaultgroupnametext', 'enrol_airtime', $a));
    $inc = 1;
    ...
    while ($DB->record_exists('groups', array('name' => $groupname, 'courseid' => $courseid))) {
        $a->increment = '(' . (++$inc) . ')';
        $newshortname = trim(get_string('defaultgroupnametext', 'enrol_airtime', $a));
