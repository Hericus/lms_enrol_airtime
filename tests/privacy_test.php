...
/**
 * Base class for unit tests for enrol_airtime.
 *
...
use enrol_airtime\privacy\provider;

/**
 * Unit tests for the enrol_airtime implementation of the privacy API.
 *
 * @copyright  2018 Carlos Escobedo <carlos@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_airtime_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_data() {
        ...
        $this->setUser($user1);
        $contextlist = provider::get_contexts_for_userid($user1->id);
        $approvedcontextlist = new approved_contextlist($user1, 'enrol_airtime', $contextlist->get_contextids());
        provider::export_user_data($approvedcontextlist);
        foreach ($contextlist as $context) {
            $writer = writer::with_context($context);
            $data = $writer->get_data([
                get_string('pluginname', 'enrol_airtime'),
                get_string('groups', 'core_group')
            ]);
            $this->assertTrue($writer->has_any_data());
            if ($context->contextlevel == CONTEXT_COURSE) {
                $exportedgroups = $data->groups;
                // User1 only belongs to group1 via enrol_airtime.
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedcontextlist = new \core_privacy\tests\request\approved_contextlist($user1, 'enrol_airtime',
    ...
    public function test_delete_data_for_users() {
        ...
        $approveduserlist = new \core_privacy\local\request\approved_userlist($coursecontext1, 'enrol_airtime',
                [$user1->id, $user2->id]);
    ...
    public function test_get_users_in_context() {
        ...
        $userlist = new \core_privacy\local\request\userlist($context, 'enrol_airtime');
        \enrol_airtime\privacy\provider::get_users_in_context($userlist);
