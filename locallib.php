...
class enrol_airtime_handler {
    ...
    public static function member_added(\core\event\cohort_member_added $event) {
        ...
        $plugin = enrol_get_plugin('airtime');
            ...
            if ($instance->customint2) {
                if (!groups_is_member($instance->customint2, $event->relateduserid)) {
                    if ($group = $DB->get_record('groups', array('id'=>$instance->customint2, 'courseid'=>$instance->courseid))) {
                        groups_add_member($group->id, $event->relateduserid, 'enrol_airtime', $instance->id);
                    }
                }
            }
        }
    ...
    public static function member_removed(\core\event\cohort_member_removed $event) {
        ...
        if (!$instances = $DB->get_records('enrol', array('customint1'=>$event->objectid, 'enrol'=>'airtime'), 'id ASC')) {
            return true;
        }
        ...
        foreach ($instances as $instance) {
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                if ($ue->status != ENROL_USER_SUSPENDED) {
                    ...
                    role_unassign_all(array('userid'=>$ue->userid, 'contextid'=>$context->id, 'component'=>'enrol_airtime', 'itemid'=>$instance->id));
                }
            }
        }
    ...
    public static function deleted(\core\event\cohort_deleted $event) {
        ...
        foreach ($instances as $instance) {
            if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                $context = context_course::instance($instance->courseid);
                role_unassign_all(array('contextid'=>$context->id, 'component'=>'enrol_airtime', 'itemid'=>$instance->id));
 ...
 */
function enrol_airtime_sync(progress_trace $trace, $courseid = NULL) {
    global $CFG, $DB;
    require_once("$CFG->dirroot/group/lib.php");
    ...
    if (!enrol_is_enabled('airtime')) {
        ...
        role_unassign_all(array('component'=>'enrol_airtime'));
        return 2;
    }
    ...
    $sql = "SELECT e.roleid, ue.userid, c.id AS contextid, e.id AS itemid, e.courseid
              FROM {user_enrolments} ue
              JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'airtime' AND e.status = :statusenabled $onecourse)
              JOIN {role} r ON (r.id = e.roleid)
              JOIN {context} c ON (c.instanceid = e.courseid AND c.contextlevel = :coursecontext)
              JOIN {user} u ON (u.id = ue.userid AND u.deleted = 0)
         LEFT JOIN {role_assignments} ra ON (ra.contextid = c.id AND ra.userid = ue.userid AND ra.itemid = e.id AND ra.component = 'enrol_airtime' AND e.roleid = ra.roleid)
    ...
    $rs = $DB->get_recordset_sql($sql, $params);
    foreach($rs as $ra) {
        role_assign($ra->roleid, $ra->userid, $ra->contextid, 'enrol_airtime', $ra->itemid);
    ...
    $sql = "SELECT ra.roleid, ra.userid, ra.contextid, ra.itemid, e.courseid
              ...
              JOIN {enrol} e ON (e.id = ra.itemid AND e.enrol = 'airtime' $onecourse)
         LEFT JOIN {user_enrolments} ue ON (ue.enrolid = e.id AND ue.userid = ra.userid AND ue.status = :useractive)
             WHERE ra.component = 'enrol_airtime' AND (ue.id IS NULL OR e.status <> :statusenabled)";
    $params = array();
    $params['statusenabled'] = ENROL_INSTANCE_ENABLED;
    $params['useractive'] = ENROL_USER_ACTIVE;
    $params['coursecontext'] = CONTEXT_COURSE;
    ...
    $rs = $DB->get_recordset_sql($sql, $params);
    foreach($rs as $ra) {
        role_unassign($ra->roleid, $ra->userid, $ra->contextid, 'enrol_airtime', $ra->itemid);
    ...
    // Finally sync groups.
    $affectedusers = groups_sync_with_enrolment('airtime', $courseid);
